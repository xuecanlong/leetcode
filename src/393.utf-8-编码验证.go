...
func validUtf8(data []int) bool {
	var i int
	for i < len(data) {
		if data[i]&128 == 0 { //对于 1 字节的字符，字节的第一位设为0，后面7位为这个符号的unicode码, 1字节第一位设为0就是128=10000000
			// 1字节
			i++
		//对于 n 字节的字符 (n > 1)，第一个字节的前 n 位都设为1，第 n+1 位设为0，后面字节的前两位一律设为10。剩下的没有提及的二进制位，全部为这个符号的unicode码
		} else if data[i]&224 == 192 { // 224=11100000, 192=11000000
			// 2字节
			if i+1 >= len(data) || data[i+1]&192 != 128 {
				return false
			}
			i += 2
		} else if data[i]&240 == 224 { // 240=11110000, 224=11100000
			// 3字节
			if i+2 >= len(data) || data[i+1]&192 != 128 || data[i+2]&192 != 128 {
				return false
			}
			i += 3
		} else if data[i]&248 == 240 { // 248=11111000
			// 4字节
